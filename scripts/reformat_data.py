...
def main(args):
    ...
    for split in ['train', 'test']:
        ds = BiomassDataset(
            ...
            month_items=True, transform=transform)
        dir_name = f'jpg_{split}_features'
        ...
        def reformat(ind):
            ...
            month = metadata['months'][0]
            x = (x.squeeze().clamp_(0, 1) * 255).to(torch.uint8)
            x = torch.vstack(tuple(x))
            skio.imsave(
                join(out_dir, f'{chip_id}_{month:02}.jpg'),
                x.numpy(), quality=90)

        thread_map(reformat, range(len(ds)), max_workers=args.max_workers, chunksize=100)
